...
data "aws_iam_policy_document" "passrole_for_unity_catalog" {
  statement {
    ...
    principals {
      identifiers = ["arn:aws-us-gov:iam::${var.ex_databricks_account_id}:role/unity-catalog-prod-UCMasterRole-1QRFA8SGY15OJ"]
      type        = "AWS"
    }
    condition {
      test     = "StringEquals"
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws-us-gov:iam::${var.aws_account_id}:role/${var.resource_prefix}-unity-catalog"]
    }
    condition {
      test     = "StringEquals"
      variable = "sts:ExternalId"
...
data "aws_iam_policy_document" "unity_catalog_iam_policy" {
  ...
  statement {
    actions   = ["sts:AssumeRole"]
    resources = ["arn:aws-us-gov:iam::${var.aws_account_id}:role/${var.resource_prefix}-unity-catalog"]
    effect    = "Allow"
  }
}
